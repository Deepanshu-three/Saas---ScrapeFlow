...
export async function ExectuteWorkflow(executionId: string, nextRunAt? : Date) {
  ...
  await finalizeExectuionWorkflow(
    executionId,
...
function setUpEnvironmentForPhase(
  ...
  for (const input of inputs) {
    ...
    //get input value from the output with environment
...
async function cleanupEnvironment(environment: Environment) {
  if (environment.browser) {
    await environment.browser
      .close()
      .catch((err) => console.error("cannot close browser: error ", err));
  }
}
